...
namespace S4PIDemoFE
{
    ...
    partial class MainForm
    {
        ...
        private void ImportSingle(string filename, string title)
        {
            ...
            if (packageExtensions.Contains(filename.Substring(filename.LastIndexOf(".")-1)))
            {
        ...
        private void ImportBatch(string[] batch, string title)
        {
            ...
            List<string> resources = batch.Where(p => !packageExtensions.Contains(p.Substring(p.LastIndexOf(".")-1))).ToList();
            List<string> packages = batch.Where(p => packageExtensions.Contains(p.Substring(p.LastIndexOf(".") - 1))).ToList();
